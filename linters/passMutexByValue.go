	...
	fn := func(node ast.Node) {
		...
		if selExpr.Sel != nil && (selExpr.Sel.Name == "Lock" || selExpr.Sel.Name == "Unlock") {
			...
			if ident, ok := selExpr.X.(*ast.Ident); ok {
				if id, ok := ident.Obj.Decl.(*ast.Field).Type.(*ast.StarExpr); !ok && id == nil {
			...
			if ident, ok := selExpr.X.(*ast.SelectorExpr); ok {
				if id, ok := ident.X.(*ast.Ident); ok {
					if expr, ok := id.Obj.Decl.(*ast.Field).Type.(*ast.StarExpr); !ok && expr == nil {
						//fmt.Println("variable: " + id.Obj.Name + " op: " + selExpr.Sel.Name)
						report.Report(pass, id.Obj, "Mutex variable: "+id.Obj.Name+"."+ident.Sel.Name+" may be passed by value.")
					}
				}
			}
		}
